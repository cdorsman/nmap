...
class TestNmapFunctional(unittest.TestCase):
    ...
    def test_basic_scan(self):
        ...
        self.assertEqual(process.returncode, 0, f"nmap command failed: {process.stderr}")
    ...
    def test_output_formats(self):
        ...
        for output_format in output_formats:
            with self.subTest(output_format=output_format):
                output_file = os.path.join(self.temp_dir.name, f'nmap_output.{output_format}')
                ...
                self.assertTrue(os.path.exists(output_file), f"Output file for {output_format} format was not created")
    ...
    def test_json_output(self):
        ...
        self.assertTrue(os.path.exists(xml_file), "XML output file for JSON conversion was not created")
        ...
        try:
            ...
            # Create a simple JSON structure mimicking the module's behavior
            json_data = {
                'scan_info': {
                    'hosts_count': 1,
                    'open_ports': [8080],
    ...
    def test_service_detection(self):
        ...
        if result['open_ports']:
            ...
            if '8080' in result['scan_services']:
                self.assertTrue(result['scan_services']['8080'].lower().startswith('http'), 
    ...
    def test_min_max_rate(self):
        ...
        process = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        self.assertEqual(process.returncode, 0, f"nmap command with rate limits failed: {process.stderr}")
...
class TestNmapFunctionalAntipatterns(unittest.TestCase):
    ...
    def test_antipattern_real_commands_without_mocks(self):
        ...
        cmd = [
            ...
            '-p', '1-65535',  # Full port scan (very slow)
        ...
        # No assertions - another antipattern
        
    def test_antipattern_dependent_tests(self):
        """ANTIPATTERN: Tests that depend on each other"""
        # Using a global variable to store state between tests
        global test_output_file
        
        # This test assumes another test has already created this variable
        try:
