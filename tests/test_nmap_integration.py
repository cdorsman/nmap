...
ANSIBLE_AVAILABLE = subprocess.run(['which', 'ansible-playbook'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0
NMAP_AVAILABLE = subprocess.run(['which', 'nmap'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0


@unittest.skipIf(not (ANSIBLE_AVAILABLE and NMAP_AVAILABLE), "ansible-playbook or nmap is not installed")
class TestNmapIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.library_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'library'))
...
class TestNmapIntegrationAntipatterns(unittest.TestCase):
    ...
    def test_antipattern_hardcoded_inventory(self):
        ...
        cmd = [
            'ansible-playbook',
            '-i', '/etc/ansible/production_inventory',  # Hardcoded path that may not exist
            f'--module-path={self.library_path}',
            playbook_path
        ]
