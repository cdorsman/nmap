...
Nmap done: 1 IP address (1 host up) scanned in 0.05 seconds
    ...
    def test_smoke_output_file(self):
        ...
        module, _ = self.create_module_mock(
            output_file=output_file, output_format="normal"
        )
...
Nmap done: 1 IP address (1 host up) scanned in 0.05 seconds
    ...
    def test_smoke_aggressive_scan(self):
        ...
        module, _ = self.create_module_mock(
            aggressive_scan=True, service_detection=True, os_detection=True
        )
    ...
    def test_smoke_json_output_creation(self):
        """Smoke test: Verify JSON output file creation"""
        json_output_file = os.path.join(self.temp_dir.name, "nmap_output.json")
        module, _ = self.create_module_mock(
            output_file=json_output_file, output_format="json"
        )

        # First check command building with JSON output
        cmd = nmap.build_nmap_command(module, "/usr/bin/nmap")

        # Verify the command has XML output (intermediate step for JSON)
        self.assertIn("-oX", cmd)
        self.assertIn(json_output_file + ".xml", cmd)

        # Now test the actual JSON creation functionality with a mock
        with patch("builtins.open", mock_open()), patch(
            "os.path.exists", return_value=True
        ), patch("os.remove") as mock_remove, patch("json.dump") as mock_json_dump:

            # Create parsed info similar to what parse_nmap_output would return
            parsed_info = {
                "hosts_count": 1,
            ...
            # Call the JSON creation code directly to ensure coverage
            try:
                # The temporary XML file
                xml_file = json_output_file + ".xml"

                # Create the JSON file with the parsed results
                ...
                # Verify the XML file cleanup was attempted
                if os.path.exists(xml_file):
                    os.remove(xml_file)
            except Exception:
                self.fail("JSON output creation raised an unexpected exception")
    ...
    def test_json_output_format(self, mock_open_func):
        ...
        # Create module with JSON output
        mock_module = self.create_module_mock(params)

        # Build command
        nmap_path = nmap.get_nmap_path(mock_module)
        cmd = nmap.build_nmap_command(mock_module, nmap_path)

        # Verify the XML output is used first
        self.assertIn("-oX", cmd)
        self.assertIn("/tmp/output.json.xml", cmd)

        # Verify that JSON conversion would happen
        with patch("os.path.exists", return_value=True):
            with patch("os.remove"):
                with patch("json.dump") as mock_json_dump:
                    # Call the function that should create the JSON file
                    result = nmap.create_json_output(
                        "/tmp/output.json",
                        {"hosts_count": 1, "open_ports": [22, 80]},
                        "nmap output",
                    )

                    # Verify function calls for JSON creation
                    self.assertTrue(result)
                    mock_open_func.assert_called_with("/tmp/output.json", "w")
                    mock_json_dump.assert_called_once()
...
class TestNmapSmokeAntipatterns(unittest.TestCase):
    ...
    def test_antipattern_not_lightweight(self):
        ...
        # Use the command to demonstrate what makes this a heavyweight operation
        self.assertIn("-p", cmd)
        self.assertIn("1-65535", cmd)  # Scanning all ports is not lightweight
        self.assertIn("-T5", cmd)  # Aggressive timing
        self.assertIn("-A", cmd)  # Aggressive scan

        # Comment explaining the antipattern
    ...
    def test_antipattern_external_dependencies(self):
        ...
        try:
            ...
            self.skipTest(
                "Requests library not installed - this is expected and demonstrates the antipattern"
            )
        ...
        if has_requests:
            try:
                ...
                response = requests.get(
                    "https://api.example.com/nmap/validate", timeout=5
                )
